...
def extract_function_body(source: str, function_name: str) -> str:
    pattern = (
        r"^\s*(?:@\w+(?:\([^)]*\))?\s+)*"
        r"(?:(?:private|fileprivate|internal|public|open|nonisolated)\s+)*"
...
class ReviewRegressionTests(unittest.TestCase):
    ...
    def test_onboarding_welcome_card_tags_stay_horizontal(self):
        source = read("QRID/QRID/Views/OnboardingView.swift")
        welcome_card = source.split("private var welcomeCard:", 1)[1].split("@ViewBuilder", 1)[0]
        self.assertIn("ScrollView(.horizontal, showsIndicators: false)", welcome_card)
        self.assertIn(".lineLimit(1)", welcome_card)
        self.assertIn(".fixedSize(horizontal: true, vertical: false)", welcome_card)

    def test_onboarding_actions_do_not_cover_fields_when_keyboard_appears(self):
        source = read("QRID/QRID/Views/OnboardingView.swift")
        self.assertNotIn('.ignoresSafeArea(.keyboard, edges: .bottom)', source)
        self.assertIn('@FocusState private var isSubtitleFocused: Bool', source)
        self.assertIn('.focused($isSubtitleFocused)', source)
        self.assertIn('proxy.scrollTo("onboarding-intro-field", anchor: .center)', source)

    def test_onboarding_rhodes_pass_offers_and_saves_landscape_banner(self):
        source = read("QRID/QRID/Views/OnboardingView.swift")
        self.assertIn('@State private var rhodesBannerPhotosItem: PhotosPickerItem?', source)
        self.assertIn('cropAspectRatio: 16.0 / 9.0', source)
        self.assertIn('if templateStyle == .rhodesPass', source)
        self.assertGreaterEqual(source.count('rhodesBannerImageData: templateStyle == .rhodesPass'), 2)

    def test_onboarding_can_change_app_language_before_setup(self):
        source = read("QRID/QRID/Views/OnboardingView.swift")
        self.assertIn('@Environment(\\.appSettings) private var settings', source)
        self.assertIn('private var onboardingLanguageMenu: some View', source)
        self.assertIn('ForEach(AppLanguage.allCases)', source)
        self.assertIn('settings.selectedLanguage = language', source)
        self.assertIn('Image(systemName: "globe")', source)
    ...
    def test_icloud_toggle_is_not_present_until_real_cloudkit_sync_exists(self):
        ...
        self.assertIn("https://meqrcode.cn/legal", about)
        self.assertIn("https://meqrcode.cn/", about)
    ...
    def test_platform_picker_uses_curated_groups_and_case_insensitive_custom_names(self):
        ...
        self.assertIn('QRLinkPolicy.platformID(url)', profile)
        self.assertIn('("testflight", ["testflight.apple.com"])', read("QRID/QRID/Helpers/QRLinkPolicy.swift"))
    ...
    def test_project_sekai_color_is_consistent_before_catalog_load(self):
        cluster = read("QRID/QRID/Models/QRCluster.swift")
        document = json.loads(read("QRID/QRID/tags-v1.json"))
        entry = next(entry for entry in document["entries"] if entry["id"] == "tag-0001")
        self.assertEqual(entry["colors"], ["#39C5BB", "#00A0E9", "#88DD44", "#FF9900", "#EE1166", "#884499"])
        self.assertEqual(entry["solidColor"], "#00A0E9")
        multicolor_defaults = cluster.split("static let multiDefaults", 1)[1].split("static let splitDefaults", 1)[0]
        self.assertIn('"projectsekai"', multicolor_defaults)
        self.assertIn('("#00A0E9", ["projectsekai"', cluster)
        renderer = read("QRID/QRID/Views/CardTagReorderView.swift")
        self.assertIn("coordinator.renderedPalette != palette", renderer)
        self.assertIn("coordinator.renderedPalette = palette", renderer)
    ...
    def test_card_tags_have_default_and_custom_colors(self):
        ...
        onboarding = read("QRID/QRID/Views/OnboardingView.swift")
        localization = read("QRID/QRID/Helpers/Localization.swift")
        remote_catalog = read("QRID/QRID/Helpers/RemoteTagCatalog.swift")
        tag_browser = read("QRID/QRID/Views/CardTagCatalogBrowser.swift")
        ...
        self.assertIn('static let onlineEnabled = true', remote_catalog)
        self.assertIn('session.data(for: request)', remote_catalog)
        tag_catalog = json.loads(read("QRID/QRID/tags-v1.json"))
        self.assertEqual(tag_catalog["revision"], tag_catalog["changelog"][0]["revision"])
        for change in tag_catalog["changelog"]:
            self.assertTrue(change["date"])
            for language in ("zhHans", "zhHantHK", "zhHantTW", "en", "ja"):
                self.assertTrue(change["summary"][language])
        self.assertEqual(len(tag_catalog["entries"]), 452)
        ...
        self.assertIn('exactEntryByKey = exactLookup', remote_catalog)
        self.assertNotIn('UserDefaults', remote_catalog)
        self.assertNotIn('private nonisolated static let entries: [Entry]', cluster)
        self.assertIn('RemoteTagCatalogSnapshot.colors(for: tag)', cluster)
        ...
        self.assertIn("Array(values.compactMap(normalizedHex).prefix(maxCustomColors))", cluster)
        self.assertIn("maxCustomColors = 5", cluster)
        self.assertIn('("#4455DD", "#33AAEE", ["星乃一歌"', cluster)
        ...
        self.assertIn('("#E53344", "#00CCAA", ["青叶摩卡"', cluster)
        self.assertIn('("#33DDAA", "#FF88BB", ["丸山彩"', cluster)
        self.assertIn('("#3344AA", "#DD2200", ["今井莉莎"', cluster)
        self.assertIn('("#FFC02A", "#AA33CC", ["濑田薰"', cluster)
        self.assertIn('("#33AADD", "#6677CC", ["仓田真白"', cluster)
        self.assertIn('("#66CC33", "#AAEE22", ["朝日六花"', cluster)
        ...
        self.assertIn('["#39C5BB", "#FFB000", "#FFE211", "#FF69B4", "#D80000", "#0068B7"]', cluster)
        ...
        self.assertIn("cluster.setTags(CardTagLimiter.tags(from: tagInput), overrides: tagColorOverrides)", edit_cluster)
        self.assertIn("struct CardTagInputView", editor)
        self.assertIn("CardTagReorderView(tags: tags", editor)
        self.assertIn("struct CardTagFlowLayout: Layout", editor)
        self.assertIn("TextField(L.tagInputHint, text: $draft)", editor)
        self.assertIn(".accessibilityLabel(L.tags)", editor)
        self.assertIn("CardTagIndex.suggestions(", editor)
        self.assertIn("guard !key.isEmpty else { return [] }", cluster)
        self.assertIn("searchEntries(for: query, excluding: existingTags, limit: 8)", cluster)
        self.assertIn("RemoteTagCatalogSnapshot.entry(matchingNormalizedKey: key)", cluster)
        self.assertIn("let solidHex = remotePreset.solidColor", cluster)
        self.assertIn(".task(id: draft)", editor)
        self.assertIn("Task.sleep(for: .milliseconds(120))", editor)
        self.assertIn('Image(systemName: "magnifyingglass")', editor)
        self.assertIn("CardTagCatalogBrowser(text: $text", editor)
        self.assertIn("struct CardTagCatalogBrowser", tag_browser)
        self.assertIn("Section(L.browseByIP)", tag_browser)
        self.assertIn("CardTagIndex.categories", tag_browser)
        self.assertIn("entries.count == 1", tag_browser)
        self.assertIn("CardTagCatalogRow(", tag_browser)
        self.assertIn(".searchable(text: $query", tag_browser)
        self.assertIn("CardTagIndex.featuredSuggestions()", onboarding)
        self.assertIn("commitDraft()", editor)
        self.assertIn("removeTag(_ tag: String)", editor)
        self.assertIn("RemoteTagCatalogSnapshot.subtitle(for: entry", editor)
        self.assertIn("selection: customColorBinding(for: tag, index: index), supportsOpacity: false)", editor)
        self.assertIn("Label(L.addColor, systemImage: \"plus.circle\")", editor)
        tag_reorder = read("QRID/QRID/Views/CardTagReorderView.swift")
        self.assertIn("UICollectionViewDragDelegate", tag_reorder)
        self.assertIn("CardTagColorPalette.colorStyle(for: tag, overrides: parent.colorOverrides)", tag_reorder)
        self.assertIn("referenceSolid ?? presetSolidHex(for: tag)", cluster)
        self.assertIn("guard hexes.count < CardTagColorPalette.maxCustomColors else { return }", editor)
        self.assertIn(".pickerStyle(.segmented)", editor)
        self.assertIn("if presetModeBinding(for: tag).wrappedValue == .custom", editor)
        self.assertNotIn("Picker(\"\", selection: presetModeBinding(for: tag))", editor)
        self.assertNotIn("CardTagColorPalette.isPresetColored(tag)", editor)
        ...
        self.assertIn("CardTagInkModifier(style:", card)
        self.assertIn(".fill(.white.opacity(cluster.cardOpacity ?? 0.7))", card)
        self.assertIn("if rhodesGlassEnabled", card)
